...
Meteor.startup(function() {
    ...
    Restaurant.Collection.Inventory._ensureIndex({
        materialId: 1,
        date: 1,
    ...
    Restaurant.Collection.StockIn._ensureIndex({
        materialId: 1
    });
    Restaurant.Collection.StockIn._ensureIndex({
        status: 1
    });
    Restaurant.Collection.Payments._ensureIndex({
        saleId: 1,
        paymentDate: 1
    }, {
        sparse: 1,
    ...
    if (Restaurant.Collection.StockEnabled.find().count() <= 0) {
        Restaurant.Collection.StockEnabled.insert({
            enableReduceStock: false,
